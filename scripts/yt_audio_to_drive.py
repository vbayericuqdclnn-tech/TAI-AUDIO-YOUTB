...
# --- XÁC THỰC GOOGLE DRIVE (Đã sửa lỗi) ---
...
def load_sa_credentials() -> Optional[service_account.Credentials]:
    ...
    if not sa_json_text: return None
    try:
...
def init_drive_service():
    """
    FIXED: Đã loại bỏ hoàn toàn luồng xác thực local/interactive (run_console)
    vì nó sẽ làm treo tiến trình trong môi trường tự động như GitHub Actions.
    Script giờ sẽ chỉ dùng credentials từ biến môi trường.
    """
...
def ensure_folder_by_id(service, folder_id: str) -> Optional[str]:
    ...
    try:
        ...
        print(f"[Drive] Không truy cập được Folder ID '{folder_id}': {e}"); return None

def drive_upload_file(service, file_path: Path, folder_id: str):
    # Logic upload giữ nguyên, đã khá tốt
    name = file_path.name
...
# --- VÒNG LẶP XỬ LÝ CHÍNH (Đã sửa lỗi) ---
